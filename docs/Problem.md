...
* **Lack of Direct and Detailed Feedback**: Standard coding platforms (e.g., LeetCode) only verify if code passes test cases. They do not evaluate code quality, complexity explanations, edge-case reasoning, or overall communication style.
...
To ensure feasibility within a 2-3 day timeline, the implementation begins with a **plain prompt-only interview engine** first. Once the prompt-only system is working, a **lightweight RAG system** is integrated. The RAG system relies on exactly one local markdown file (`backend/data/knowledge.md`) with a built-in fallback: if the vector database or retrieval fails, the backend falls back to standard Gemini API prompts without context.

Upon receiving the user's answer, the system executes an automated qualitative and quantitative evaluation, returning an immediate score, targeted feedback, and specific suggestions for improvement. The session history and question logs are saved in a local **SQLite** database and displayed on a simple **React** dashboard.

## 4. Objectives
* **Provide Accessible Interview Prep**: Give job seekers a free, locally runnable, and realistic mock interview environment.
* **Establish Relevant Context (RAG)**: Retrieve authentic question-answer patterns and concept frameworks for Python, DSA, and HR interviews from a single source file to ensure high question quality and strict evaluation.
...
* **Ensure Timeline Feasibility**: Keep the design clean, modular, and achievable within a 2–3 day sprint by using lightweight, modern technologies (FastAPI, SQLite, React, ChromaDB) and implementing a step-by-step phased plan (prompt-only first, then RAG).
...
2. **Single-Question Loop**: The interview session is cut to exactly **one question**. The workflow is: Select Topic -> Generate Question 1 -> Submit Answer -> Get Feedback & Score -> Save & Complete -> Dashboard.
3. **Simplified UI**: Consists of four basic pages (Home Page, Topic Selection, Interview Page, Dashboard) with clean layout components (no complex chat layers, typing animations, split panes, or code highlights).
4. **Lightweight RAG**: Ingests exactly one markdown file (`backend/data/knowledge.md`).
5. **Robust Fallback**: If the local RAG retrieval fails, the system falls back gracefully to a prompt-only Gemini query.
...
* **Company & Job Role Profiles**: Curating custom vector indexes loaded with real-world interview leaks for specific top-tier companies.
